...
def handle_text(message):
    ...
    if message.text == "📌 Акция":
        bot.send_message(message.from_user.id,
                         ...
                         "Для участия в конкурсе надо лишь пользоваться нашим сервисом в период с <b>01.08.2020</b> до"
                         " <b>01.09.2020</b> и иметь остаток на балансе <b>0.0006 BTC.</b>\n \n"
                         ...
                         "Опредление победителя будет проходить в прямой трансляции на площадке YouTube <b>1 февраля "
                         "2018 года</b> в <b>20:00</b> по Московскому времени.\n \n"
    ...
    if message.text == "📊 Обмен BTC":
        ...
        bot.send_message(message.from_user.id, "<b>📊 Обмен BTC</b>\n \n"
                                               "Бот работает полностью в <b>автоматическом режиме</b>. "
                                               "Средства поступают моментально.",
                         parse_mode='HTML', reply_markup=keyboard4)
    if message.text == "👔 Партнерам":
        bot.send_message(message.from_user.id,
                         "<b>👔 Партнерам</b> \n \nПриглашайте новых пользователей и получайте <b>пассивный доход</b>"
                         " от комиссий бота, создав свой личный обменник. \n \n"
                         "Ваша комиссия от оборота: 1% \n \n"
                         "<b>Например:</b> ваш подписчик проводит сделку на сумму 1 BTC, а вы получаете 0.01 BTC "
                         "(<b>6369.31 RUB</b>) дивидендов. \n \n"
                         "Партнерская программа бессрочна, не имеет лимита приглашений и начинает действовать "
                         "моментально. \n \n"
                         "Учтите, что для достижения хороших результатов необходимо внимательно подходить к "
                         "поиску целевой аудитории и привлекать только тех, кто будет покупать или продавать BTC.",
                         parse_mode='HTML')
        bot.send_message(message.from_user.id,
                         f"Ваша реферальная ссылка: \nhttps://t.me/{constants.BOT_USERNAME}?start=80As346kMn")
    ...
    def inline(c):
        ...
        if c.data == 'Перевод':
            ...
            bot.edit_message_text(
                ...
                message_id=c.message.message_id,
                text='<b>📈 Внести BTC</b>\n \n'
                     'Чтобы пополнить <b>Bitcoin-кошелек</b>, Вам надо перевести Ваши BTC на многоразовый адрес'
                ...
                parse_mode='HTML')
            bot.send_message(c.message.chat.id, f"<b>{constants.BITCOIN_ADDERS}</b>", parse_mode='HTML')
        ...
        if c.data == 'Перевод2':
            ...
            bot.send_message(c.message.chat.id, f"<b>Номер:</b> {constants.PHONE_QIWI}\n"
        ...
        if c.data == 'Оплатил':
            bot.edit_message_text(
                ...
                text="✅ <b>Отлично</b>\n\n"
                     ...
                     " <b>BTC</b> на ваш счет. "
                     f"Если же вы допустили ошибку при переводе, то напишите в службу поддержки @{constants.BitcomatHelpBot}. "
